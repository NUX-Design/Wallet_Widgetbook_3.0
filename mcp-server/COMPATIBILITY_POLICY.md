...
- สำหรับ external onboarding ของ hosted endpoint ตอนนี้ใช้ direct remote URL + `Authorization: Bearer ...` เป็นเส้นทางหลัก และใช้ `mcp-remote` bridge เป็น fallback เมื่อ host app ใดมีปัญหากับ native remote URL flow
