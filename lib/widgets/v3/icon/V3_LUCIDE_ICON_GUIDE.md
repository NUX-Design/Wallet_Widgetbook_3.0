...
| [`110:5381`](https://www.figma.com/design/mhUvPg9tOjlvQvEW6glQhJ/Wi-Design-System?node-id=110-5381) | `V3Navigation` Menu (inactive / selected) | `LucideIcons.menu` | `medium` (24px) | `regular` inactive / `bold` selected | package | — |
...
Selected/inactive distinction: Lucide has no filled/outline icon pairs (it is a stroke-only icon set), so `V3NavigationPreview` mirrors the existing Bold/Medium label-weight pairing documented in `V3_NAVIGATION_GUIDE.md` (`Active label ใช้ label/tiny Bold ... inactive ใช้ label/tiny Medium`) by rendering the selected destination icon at `V3IconStroke.bold` and the inactive icon at the `V3IconStroke.regular` default — confirmed visually in both Light and Dark via a real headless-Chrome screenshot of `http://127.0.0.1:8100/#/navigation/V3Navigation` (see LI-06 evidence in `task/V3_LUCIDE_ICON_TASKS.md`). This is a deliberate design decision made in this session, not a value read off Figma; if Figma later specifies an exact selected-icon treatment, update this table and the preview together.
