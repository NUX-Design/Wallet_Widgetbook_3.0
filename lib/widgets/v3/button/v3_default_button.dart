...
enum V3DefaultButtonVariant { primary, outline, ghost }

enum V3DefaultButtonState { defaultState, active, disabled, error }

/// Theme V3 Default (largest) button mapped to Wi Design System Figma node
/// `504:1780` (`Size=Default`), sharing color tokens with
/// [V3DefaultButtonVariant] per `button.md`'s "Color tokens are independent
/// of size" invariant. Figma's `Size=Default` option is the 48px tier, the
/// largest of the four — not the API's actual default variant (`mini`).
class V3DefaultButton extends StatelessWidget {
  const V3DefaultButton({
    super.key,
    required this.label,
    this.onPressed,
    this.variant = V3DefaultButtonVariant.primary,
    this.state = V3DefaultButtonState.defaultState,
    this.leadingIcon,
    this.trailingIcon,
  ...
  final V3DefaultButtonVariant variant;
  ...
  bool get _isEnabled =>
      onPressed != null && !isLoading && state != V3DefaultButtonState.disabled;

  @override
  Widget build(BuildContext context) {
    final colors = V3ThemeScope.colorsOf(context);
    final metrics = _V3DefaultButtonMetrics.forVariant(variant);
    final effectiveLeading =
        isLoading
            ? SizedBox.square(
              dimension: metrics.iconSize,
              child: CircularProgressIndicator(
                key: const ValueKey('v3-default-button-progress'),
                strokeWidth: 2,
    ...
    Widget button = Semantics(
      button: true,
      enabled: _isEnabled,
      toggled: state == V3DefaultButtonState.active ? true : null,
      label: semanticLabel ?? label,
      hint: semanticHint,
      child: ExcludeSemantics(
        child: DecoratedBox(
          key: const ValueKey('v3-default-button-decoration'),
  ...
  ButtonStyle _buttonStyle(
    ...
    _V3DefaultButtonMetrics metrics,
  ) {
    return ButtonStyle(
      ...
      textStyle: WidgetStatePropertyAll(
        V3Typography.labelMedium.copyWith(
          decoration:
              variant == V3DefaultButtonVariant.ghost
  ...
  List<BoxShadow> get _boxShadow {
    final hasShadow =
        ...
        (state == V3DefaultButtonState.defaultState ||
            state == V3DefaultButtonState.active);
  ...
  Color _foregroundColor(V3ColorPalette colors, Set<WidgetState> states) {
    if (_isDisabled(states)) {
      return variant == V3DefaultButtonVariant.ghost
          ? colors.contentNeutral
          : colors.contentNeutral2;
    }
    if (variant == V3DefaultButtonVariant.primary) {
      return colors.contentWhite;
    }
    if (variant == V3DefaultButtonVariant.outline) {
  ...
  Color _backgroundColor(V3ColorPalette colors, Set<WidgetState> states) {
    if (variant == V3DefaultButtonVariant.ghost) {
    ...
    if (_isError) {
      return variant == V3DefaultButtonVariant.primary
          ? colors.stateError
  ...
  BorderSide _borderSide(V3ColorPalette colors, Set<WidgetState> states) {
    if (variant != V3DefaultButtonVariant.outline || _isDisabled(states)) {
  ...
  bool get _isError => state == V3DefaultButtonState.error;
...
class _ButtonContent extends StatelessWidget {
  ...
  final Widget? leadingIcon;
  final Widget? trailingIcon;
  final _V3DefaultButtonMetrics metrics;
...
class _V3DefaultButtonMetrics {
  const _V3DefaultButtonMetrics({
  ...
  /// `504:1780` `Size=Default` height (48px) has no bound V3 dimension
  /// token in the current Figma export (`height.token == null` for every
  /// size in `button-_base.json`); kept as a literal to stay Figma-traceable.
  factory _V3DefaultButtonMetrics.forVariant(V3DefaultButtonVariant variant) {
    return variant == V3DefaultButtonVariant.ghost
        ? const _V3DefaultButtonMetrics(
        ...
        : const _V3DefaultButtonMetrics(
          height: 48,
          horizontalPadding: V3Spacing.space32,
          verticalPadding: V3Spacing.space12,
          gap: V3Spacing.space8,
        );
  }
