...
import 'package:mcp_test_app/widgets/v3/button/v3_default_button.dart';
...
void main() {
  Future<void> pumpButton(
    ...
    V3DefaultButtonVariant variant = V3DefaultButtonVariant.primary,
    V3DefaultButtonState state = V3DefaultButtonState.defaultState,
    Widget? leadingIcon,
    Widget? trailingIcon,
  ...
  testWidgets('maps all Primary Default states to Figma tokens', (
    tester,
  ) async {
    ...
    await pumpButton(tester, state: V3DefaultButtonState.disabled);
    expect(
    ...
    await pumpButton(
      tester,
      state: V3DefaultButtonState.error,
      onPressed: () {},
    );
    expect(
      buttonStyle(tester).backgroundColor!.resolve({}),
  ...
  testWidgets(
    'maps all Outline Default states including dedicated error border',
    (tester) async {
      await pumpButton(
        tester,
  ...
  testWidgets('maps all Ghost Default states and underline style', (
    tester,
  ) async {
    for (final state in V3DefaultButtonState.values) {
      await pumpButton(
        ...
        state: state,
        onPressed: state == V3DefaultButtonState.disabled ? null : () {},
      );
  ...
  testWidgets('matches exact Default metrics and icon slots', (tester) async {
    await pumpButton(
      tester,
    ...
    expect(
      ...
      const EdgeInsets.symmetric(
        horizontal: V3Spacing.space32,
        vertical: V3Spacing.space12,
      ),
    );
    expect(textStyle.fontSize, V3Typography.labelMedium.fontSize);
    expect(textStyle.height, V3Typography.labelMedium.height);
    expect(textStyle.fontWeight, V3Typography.labelMedium.fontWeight);
    expect(
  ...
  testWidgets('uses radius and shadow tokens for Outline Figma states', (
    ...
    var decoration =
        tester
                .widget<DecoratedBox>(
                  find.byKey(const ValueKey('v3-default-button-decoration')),
                )
                .decoration
            as BoxDecoration;
  ...
  testWidgets('disabled and loading states do not invoke callbacks', (
    ...
    await pumpButton(
      tester,
      state: V3DefaultButtonState.disabled,
      onPressed: () => presses++,
    );
    expect(
    ...
    expect(
      find.byKey(const ValueKey('v3-default-button-progress')),
      findsOneWidget,
    );
    await tester.tap(find.byType(V3DefaultButton));
    expect(presses, 0);
  });
  ...
  testWidgets('selects Dark mode Default button semantics', (tester) async {
    await pumpButton(
      tester,
  ...
  testWidgets(
    ...
    (tester) async {
      ...
      final expectedCount =
          V3DefaultButtonVariant.values.length *
          V3DefaultButtonState.values.length *
          2;
  ...
  testWidgets('Default label remains stable at large text scale', (
    tester,
  ) async {
